...
def main(PID = os.getpid(), FIND_STR='ąsdf1234', _exit_on_first_match=True):
 ...
 procHandle = Kernel32.OpenProcess(PROCESS_QUERY_INFORMATION|PROCESS_VM_READ|PROCESS_VM_WRITE, False, PID) # print('process:', Process)
    ...
    if rm>0 :
     ...
     if buffer.value==_x:
      ...
      if ci==len(findstr):
       ...
       buffer.value=replacestr[0].to_bytes(1,'little')
       wrm=Kernel32.WriteProcessMemory(procHandle, ctypes.c_void_p(f_ind), ctypes.byref(buffer),  ctypes.sizeof(buffer), ctypes.byref(nread)) # read cur region bytes to bugger
       print(f"{wrm}, {nread}")
